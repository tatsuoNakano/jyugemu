...
class HistoryManager {
    constructor(historyFilePath = 'jyugemu.json') {
        this.fileManager = new file_manager_1.FileManager(historyFilePath);
    }
    /**
     ...
     */
    async displayHistory(limit = 50, filter) {
        try {
            ...
            throw new Error(`Failed to display history: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
    ...
    formatForDisplay(records) {
        ...
        const maxTimestampLen = Math.max(...records.map((r) => r.timestamp.length), 'Timestamp'.length);
        const maxCommandLen = Math.max(...records.map((r) => r.command.length), 'Command'.length);
        const maxExitCodeLen = Math.max(...records.map((r) => r.exit_code.toString().length), 'Exit Code'.length);
