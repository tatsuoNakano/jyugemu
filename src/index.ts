...
class JyugemuApp {
   ...
   */
  async handleInit(force: boolean = false): Promise<void> {
    const fileManager = new FileManager(this.historyFilePath);
    ...
    try {
      ...
      if (fs.existsSync(this.historyFilePath) && !force) {
        ...
        const confirmed = await this.promptUserConfirmation(
          `History file already exists at ${this.historyFilePath}. Overwrite? (y/n): `
        );
      ...
      console.error(
        `✗ Failed to initialize history file: ${error instanceof Error ? error.message : String(error)}`
      );
      process.exit(1);
    }
  }
  ...
  async handleStart(): Promise<void> {
    ...
    if (!fs.existsSync(this.historyFilePath)) {
      console.error(
        `✗ History file not found at ${this.historyFilePath}. Run 'jyugemu init' first.`
      );
      process.exit(1);
    }
   ...
   */
  async handleHistory(limit: number = 50, filter?: string): Promise<void> {
    // Check if history file exists
    if (!fs.existsSync(this.historyFilePath)) {
      console.error(
        `✗ History file not found at ${this.historyFilePath}. Run 'jyugemu init' first.`
    ...
    try {
      const historyManager = new HistoryManager(this.historyFilePath);
   ...
   */
  private async promptUserConfirmation(question: string): Promise<boolean> {
...
async function main(): Promise<void> {
  try {
    ...
    const parsedCommand = parseArgs(args);
